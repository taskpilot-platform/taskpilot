...
@Tag(name = "Personal Skills", description = "APIs for managing user skills")
@RestController
...
public class SkillController {
    ...
    @GetMapping("/{skill_id}")
    public ApiResponse<UserSkillResponseDTO> getSkillDetail(@PathVariable("skill_id") Long skillId) {
        return ApiResponse.success(HttpStatus.OK.value(), "Skill retrieved successfully", skillService.getSkillDetail(skillId));
    }

    @Operation(summary = "Add Skill", description = "Add a new skill or existing skill for the current user.")
    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<Void> addSkill(@Valid @RequestBody AddSkillRequestDTO request) {
    ...
    @PutMapping("/{skill_id}")
    public ApiResponse<Void> updateSkill(@PathVariable("skill_id") Long skillId, @Valid @RequestBody UpdateSkillRequestDTO request) {
