-- Migration V22: Migrate identity columns to dedicated PostgreSQL sequences
...
BEGIN
    FOREACH tbl IN ARRAY tbl_list LOOP
        IF to_regclass(tbl) IS NOT NULL THEN
            ...
            -- 1. Create explicit sequence if not exists
            EXECUTE format('CREATE SEQUENCE IF NOT EXISTS %I', seq);

            -- 2. Synchronize current sequence value with existing table data
            ...
            -- 3. Drop identity constraint if column was generated as identity
            EXECUTE format('ALTER TABLE %I ALTER COLUMN id DROP IDENTITY IF EXISTS', tbl);
